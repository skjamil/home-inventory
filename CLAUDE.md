...
- `ARCHITECTURE.md` — tech stack with rationale, system diagram, project folder structure, key data flows (file upload, auth, expiration notification), the Tailwind/mobile-first responsive strategy, required env vars.
...
- **Auth is self-service and multi-user**: Auth.js credentials provider + JWT sessions (Credentials only supports JWT, not database sessions — a hard library constraint, confirmed at runtime), required email verification before first login, email-based password reset, rate limiting (Upstash) on `register`/`login`/`forgot-password`/`resend-verification`. Route protection is layered — a lightweight Edge middleware fast-path, plus the real enforcement in `(protected)/layout.tsx` and every protected API route (Prisma isn't Edge-compatible, so middleware alone can't do it). Resend sends verification/reset emails and the daily expiration-notification digest (Mailtrap via `nodemailer` stands in for Resend in local dev — see `ARCHITECTURE.md`). Expiration notifications (warranty + AMC) go through three channels: a live-polling dashboard banner, this daily email digest, and browser push (Web Push API, per-device) — all driven by one canonical threshold helper (`lib/expiry.ts`) and a daily Vercel Cron job, detailed in `ARCHITECTURE.md`'s "Expiration notification" section.
