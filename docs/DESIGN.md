...
- **Receipt scan auto-fill**: capturing/replacing the Receipt photo runs free, client-side OCR (Tesseract.js) in the background — a "Scanning receipt…" indicator shows while it runs, then a dismissible "Auto-filled from the receipt — please review before saving" notice appears if Name, Price, or Purchase date were filled in. Only fields the user hasn't already typed get filled; a failed or inconclusive scan leaves the form untouched with no error shown.
