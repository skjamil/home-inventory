...
- **`lib/expiry.ts`** is the single source of truth for the 30/7/1-day threshold math behind the email/push channels: `getExpiryStatus(date)` returns `{ daysUntil, isExpired, isExpiringSoon }`, consumed by the item detail page and the cron job below. It does **not** back the in-app banner (see next bullet) — the two intentionally use different windows.
- **In-app live banner**: `lib/expiring-entries.ts`'s `getExpiringEntries(userId)` — backed by `lib/warranty.ts`/`lib/amc.ts`, which scope to the **current calendar month** (`startOfMonth`/`endOfMonth`), not the 30/7/1-day thresholds above, and not open-ended for already-expired items — an item that expired last month drops off the banner on the 1st. Gated by `warrantyNotificationsEnabled`/`amcNotificationsEnabled`, backs both the dashboard's SSR initial paint and `GET /api/notifications/expiring`, which `ExpirationBanner` polls every 60s (paused while the tab is hidden, refetched immediately on refocus) so the banner updates without a manual reload.
